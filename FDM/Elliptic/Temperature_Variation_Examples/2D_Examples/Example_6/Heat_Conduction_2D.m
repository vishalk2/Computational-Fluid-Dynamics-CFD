% 2D Heat Conduction & Convection of a plate
clear all; clc

% Known Values
Lx = 1; % Length of x-domain
Ly = 2; % Length of y-domain
M = 21; % Number of grid points in x-direction
N = 41; % Number of grid points in y-direction
T0 = 273; % Free Air temperature
...
while err_Jacobi>epsilon
    % For Applying Boundary Conditions(BC)
    for i=1:M
        % Top BC
        T_Jacobi(1,i) = (4*T_Jacobi(2,i)-T_Jacobi(3,i)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        % Bottom BC
        T_Jacobi(N,i) = (4*T_Jacobi(N-1,i)-T_Jacobi(N-2,i)-(2*h*dy/k)*T0)/(3-(2*h*dy/k));
    end
    for j=1:N
        % Right BC
        T_Jacobi(j,M) = (4*T_Jacobi(j,M-1)-T_Jacobi(j,M-2)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        %Left BC
        T_Jacobi(j,1) = (4*T_Jacobi(j,2)-T_Jacobi(j,3))/3;
    end
    
    for j=11:31 % Left BC (from node=11 to node=31)
        T_Jacobi(j,1) = 303;
    end
    
    % Finding T_Jacobi from previous iteration i.e. finding T_old_Jacobi
    for j=1:N
        for i=1:M
    ...
    err_Jacobi = 0; % Error
    for j=1:N % Error calculation after each iteration
        for i=1:M
...
while err_GS>epsilon
    ...
    for i=1:M
        % Top BC
        T_GS(1,i) = (4*T_GS(2,i)-T_GS(3,i)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        % Bottom BC
        T_GS(N,i) = (4*T_GS(N-1,i)-T_GS(N-2,i)-(2*h*dy/k)*T0)/(3-(2*h*dy/k));
    end
    for j=1:N
        % Right BC
        T_GS(j,M) = (4*T_GS(j,M-1)-T_GS(j,M-2)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        %Left BC
        T_GS(j,1) = (4*T_GS(j,2)-T_GS(j,3))/3;
    end
...
B = dx/dy;
w = 1.9; % Optimum Relaxation Factor
...
while err_PSOR>epsilon
    ...
    for i=1:M
        % Top BC
        T_PSOR(1,i) = (4*T_PSOR(2,i)-T_PSOR(3,i)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        % Bottom BC
        T_PSOR(N,i) = (4*T_PSOR(N-1,i)-T_PSOR(N-2,i)-(2*h*dy/k)*T0)/(3-(2*h*dy/k));
    end
    for j=1:N
        % Right BC
        T_PSOR(j,M) = (4*T_PSOR(j,M-1)-T_PSOR(j,M-2)+(2*h*dy/k)*T0)/(3+(2*h*dy/k));
        
        %Left BC
        T_PSOR(j,1) = (4*T_PSOR(j,2)-T_PSOR(j,3))/3;
    end
