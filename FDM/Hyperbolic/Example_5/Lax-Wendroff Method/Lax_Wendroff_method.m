...
for j=1:length(Dt)
    ...
    % Computation using Lax-Wendroff Method
    t = 0; % Time
    n = 0; % Iterator
    while t<=2
        U_old = U;
        F = (U.^2)/2;
        for i=2:M-1
            U(1,i) = U_old(1,i)-(dt/(2*dx))*(F(1,i+1)-F(1,i-1))+((dt^2)/(4*dx^2))*((U_old(1,i+1)+U_old(1,i))*(F(1,i+1)-F(1,i))-(U_old(1,i)+U_old(1,i-1))*(F(1,i)-F(1,i-1)));
        end
        t = t+dt;
        n = n+1; % Iterator
    ...
    title('Velocity Profile - Lax-Wendroff Method',strcat('Time Step :',num2str(dt)))
