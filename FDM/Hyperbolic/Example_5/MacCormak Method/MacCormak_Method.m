...
for j=1:length(Dt)
    ...
    Up = U; % Velocity for Predictor Step
    
    % Computation using MacCormak Method
    t = 0; % Time
    n = 0; % Iterator
    while t<=2
        U_old = U;
        for i=2:M-1
            % Predictor Step
            Up(1,i) = U_old(1,i)-(0.5*dt/dx)*(U_old(1,i+1).^2-U_old(1,i).^2);
        end
        for i=2:M-1
            % Corrector Step
            U(1,i) = (1/2)*(U_old(1,i)+Up(1,i)-(0.5*dt/dx)*(Up(1,i).^2-Up(1,i-1).^2));
        end
        t = t+dt;
        n = n+1; % Iterator
    ...
    title('Velocity Profile - MacCormak Method',strcat('Time Step :',num2str(dt)))
